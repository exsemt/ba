ActiveSupport::Notifications.subscribe "sql.active_record" do |*args|
  ...
  if SUBSCRIBE['SQL'] && (event.payload[:name] && event.payload[:name].is_a?(Hash) && event.payload[:name][:model].include?("case")) && "SCHEMA" != event.payload[:name] &&
    ...
    sql = %{INSERT INTO `sql_requests` (`server_id`,`start`,`finish`,`sql_duration`,`name`,`scenario_case`,`sql_query`,`payload`,`table_size`)
      (SELECT '#{event.transaction_id}',
        '#{event.time.strftime("%Y-%m-%d %H:%M:%S")}',
        '#{event.end.strftime("%Y-%m-%d %H:%M:%S")}',
        '#{event.duration}',
        '#{event.payload[:name][:model]}',
        '#{event.payload[:name][:scenario_case]}',
