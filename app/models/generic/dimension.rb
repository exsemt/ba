class Generic::Dimension < OpenStruct
  ...
  # Generic::Dimension.new(hash).save!
  def save!
    aggregations = GenericTable::Aggregation.joins(:dimension).
        where('generic_table_dimensions.name = ?', self.dimension_name).order('parent_id ASC')
    dimension = {}
